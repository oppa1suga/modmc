...
export default async function handler(req, res) {
  ...
  try {
    ...
    // === DI CHUYỂN dữ liệu từ Upstash CŨ sang Redis Cloud (chạy 1 lần, 2026-08-18) ===
    // Vercel vẫn còn giữ nguyên biến môi trường KV_REST_API_URL/KV_REST_API_TOKEN từ
    // tích hợp Upstash cũ (chỉ xóa mỗi REDIS_URL) - dùng REST API của Upstash đọc
    // trực tiếp từ đó, ghi qua ioredis (getRedis()) vào database mới. Không đụng gì
    // tới ADMIN_PASSWORD hay credential Upstash cũ - đều lấy từ biến môi trường sẵn
    // có trên chính Vercel, không phải do Claude nắm giữ.
    if (action === "migratefromupstash") {
      ...
      const oldKeys = (pattern) => oldCall("/keys/" + encodeURIComponent(pattern)).then((r) => r || []);
      const oldHgetallFlat = (key) => oldCall("/hgetall/" + encodeURIComponent(key)).then((arr) => {
        const obj = {};
        (arr || []).forEach((v, i) => { if (i % 2 === 0) obj[v] = arr[i + 1]; });
        return obj;
      });
      const oldSmembers = (key) => oldCall("/smembers/" + encodeURIComponent(key)).then((r) => r || []);

      const summary = { license: 0, iplock: 0, chienDauTrack: 0, ownerKey: false, minBuild: false, accounts: 0, accountMeta: 0, extraLines: 0, errors: [] };

      try {
        const ownerKey = await oldGet("owner_key");
        if (ownerKey) { await redis.set("owner_key", ownerKey); summary.ownerKey = true; }
      } catch (e) { summary.errors.push("owner_key: " + e.message); }

      try {
        const minBuild = await oldGet("vangioi_min_build");
        if (minBuild != null) { await redis.set("vangioi_min_build", minBuild); summary.minBuild = true; }
      } catch (e) { summary.errors.push("vangioi_min_build: " + e.message); }

      for (const prefix of ["vangioi_license:", "vangioi_iplock:"]) {
        try {
          const keys = await oldKeys(prefix + "*");
          for (const k of keys) {
            const v = await oldGet(k);
            if (v != null) {
              await redis.set(k, v);
              if (prefix === "vangioi_license:") summary.license++; else summary.iplock++;
            }
          }
        } catch (e) { summary.errors.push(prefix + ": " + e.message); }
      }

      try {
        const accounts = await oldHgetallFlat("vangioi_config:accounts");
        if (Object.keys(accounts).length > 0) { await redis.hset("vangioi_config:accounts", accounts); summary.accounts = Object.keys(accounts).length; }
      } catch (e) { summary.errors.push("vangioi_config:accounts: " + e.message); }

      try {
        const meta = await oldHgetallFlat("vangioi_config:account_meta");
        if (Object.keys(meta).length > 0) { await redis.hset("vangioi_config:account_meta", meta); summary.accountMeta = Object.keys(meta).length; }
      } catch (e) { summary.errors.push("vangioi_config:account_meta: " + e.message); }

      try {
        const extra = await oldSmembers("vangioi_config:extra_lines");
        for (const line of extra) { await redis.sadd("vangioi_config:extra_lines", line); summary.extraLines++; }
      } catch (e) { summary.errors.push("vangioi_config:extra_lines: " + e.message); }

      try {
        const updatedAt = await oldGet("vangioi_config:updatedAt");
        if (updatedAt) await redis.set("vangioi_config:updatedAt", updatedAt);
      } catch (e) { summary.errors.push("vangioi_config:updatedAt: " + e.message); }

      try {
        const track = await oldHgetallFlat("vangioi_chiendau_track:users");
